...
export default function Example() {
  ...
  React.useEffect(()=>{
    console.log(cep)
    if (cep.length > 8) {
      searchAddress(cep.match( /\d+/g ).join(''));
    }
  },[cep])

  const searchAddress = async (num_cep) =>  {
    console.log(num_cep)
    await fetch(`${process.env.REACT_APP_API_BACKEND_URL}/cep/${num_cep}`, {
       method: 'GET',
       headers: {
 ...
 const saveCustomer = async (nome, email) =>  {
...
const handleSubmit = (e) => {
  e.preventDefault();
  setUser(saveCustomer(nome, email, documento, telefone));
};  
